...
const Tasks01 = ({
    ...
    useEffect(()=>{
        ...
        if(isMounted){
            InspectionAPI.getMemberProfileDetails(assigned_by).then((res)=>{
                setMember(res)
            }).catch((err)=>{
                console.log(err)
            })
    ...
    return (
        <View style={[styles.contain, style, { backgroundColor: colors.card, marginTop:0, marginBottom:-8 }]}>
            <View style={{ flex: 1,  borderBottomWidth:1, borderBottomColor:"rgba(0,0,0,0.1)", paddingBottom:20 }}>
                ...
                >
                    <Avatars member={member} textstyle="body1"/>
                </View>
                {stats.todo>0 &&
                <>
